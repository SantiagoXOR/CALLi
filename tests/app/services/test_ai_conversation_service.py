...
async def test_process_message(ai_conversation_service):
    ...
    ai_conversation_service.analyze_sentiment = AsyncMock(
        return_value={"primary_emotion": "neutral", "score": 0.5}
    )
...
async def test_analyze_sentiment(ai_conversation_service):
    ai_conversation_service.llm.ainvoke = AsyncMock(
        return_value='{"primary_emotion": "positive", "score": 0.8}'
    )
...
async def test_get_from_cache(ai_conversation_service):
    ai_conversation_service.get_from_cache = AsyncMock(
        return_value=[
...
async def test_set_in_cache(ai_conversation_service):
    ...
    await ai_conversation_service.set_in_cache(
        "123",
        [
            {"type": "human", "content": "Hola"},
            {"type": "ai", "content": "Hola, ¿en qué puedo ayudarte?"},
    ...
    ai_conversation_service.set_in_cache.assert_called_once_with(
        "123",
        [
            {"type": "human", "content": "Hola"},
            {"type": "ai", "content": "Hola, ¿en qué puedo ayudarte?"},
