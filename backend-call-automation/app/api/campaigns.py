...
async def create_campaign_endpoint(
    campaign: CampaignCreate, db: AsyncSession = Depends(get_session)
) -> Campaign:
...
async def get_campaign_endpoint(
    campaign_id: UUID, db: AsyncSession = Depends(get_session)
) -> Campaign:
...
async def update_campaign_endpoint(
    campaign_id: UUID, campaign_update: CampaignUpdate, db: AsyncSession = Depends(get_session)
) -> Campaign:
...
async def delete_campaign_endpoint(campaign_id: UUID, db: AsyncSession = Depends(get_session)):
...
async def add_client_to_campaign_endpoint(
    campaign_id: UUID, client_id: UUID, db: AsyncSession = Depends(get_session)
):
    try:
