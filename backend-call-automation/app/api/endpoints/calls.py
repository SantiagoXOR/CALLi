    ...
    try:
        ...
        while True:
            ...
            audio_response = await call_service.handle_call_response(
                call_id=call_id, user_message=user_message
            )
...
async def audio_websocket_endpoint(websocket: WebSocket, call_id: str) -> None:
    """
    ...
    try:
        ...
        await call_service.elevenlabs_service.configure_stream_settings(
            {"chunk_size": 1024, "latency": "low", "quality": "high"}
        )
        ...
        while True:
            ...
            response_chunk = await call_service.handle_audio_stream(
                call_id=call_id, audio_chunk=audio_chunk
            )
        ...
        logging.exception(f"Error en websocket de audio: {e!s}")
        await call_service.end_call(call_id)
        raise
