...
class AIConfig(BaseModel):
    ...
    temperature: float = Field(
        default=0.7, ge=0, le=1, description="Temperatura para la generación"
    )
    ...
    custom_prompt_variables: dict[str, str] = Field(
        default_factory=dict, description="Variables personalizadas para el prompt"
    )
...
class CampaignBase(BaseModel):
    ...
    status: CampaignStatus = Field(
        default=CampaignStatus.DRAFT, description="Estado actual de la campaña"
    )
    ...
    contact_list_ids: list[UUID] = Field(
        default_factory=list, description="Lista de IDs de contactos objetivo"
    )
    script_template: str = Field(
        ..., min_length=10, description="Plantilla para el script de llamada"
    )
    max_retries: int = Field(default=3, ge=0, description="Número máximo de reintentos por llamada")
    retry_delay_minutes: int = Field(
        default=60, ge=0, description="Tiempo de espera entre reintentos en minutos"
    )
    ...
    ai_config: AIConfig = Field(
        default_factory=AIConfig, description="Configuración específica para la IA"
    )
    key_points: list[str] = Field(
        default_factory=list, description="Puntos clave a mencionar durante la llamada"
    )

    model_config = ConfigDict(from_attributes=True)

    @field_validator("schedule_end")
    def end_must_be_after_start(cls, v: datetime, info: ValidationInfo) -> datetime:
    ...
    def validate_calling_hours(cls, v: str, info: ValidationInfo) -> str:
...
class CampaignStats(BaseModel):
    ...
    def validate_call_counts(cls, v: int, info: ValidationInfo) -> int:
        if (
            "total_calls" in info.data
            and "successful_calls" in info.data
            and "failed_calls" in info.data
        ):
            ...
            if successful + failed > total:
                raise ValueError(
                    "La suma de llamadas exitosas y fallidas no puede exceder el total"
                )
        return v
