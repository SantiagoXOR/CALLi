...
from app.dependencies import get_call_service, get_campaign_service, get_contact_service
...
async def get_campaign_summary(
    ...
    success_rate = (
        (call_metrics.get("by_status", {}).get("completed", 0) / total_calls) * 100
        if total_calls > 0
        else 0
    )
    failure_rate = (
        (call_metrics.get("by_status", {}).get("failed", 0) / total_calls) * 100
        if total_calls > 0
        else 0
    )
...
async def get_campaigns_performance(
    ...
    for campaign in campaigns:
        ...
        call_metrics = await call_service.get_call_metrics(
            campaign_id=campaign.id, start_date=start_date, end_date=end_date
        )
...
async def get_performance_metrics(
    ...
    metrics = await call_service.get_call_metrics(
        campaign_id=campaign_id, start_date=start_date, end_date=end_date, group_by="day"
    )
    ...
    return {
        ...
        "summary": {
            ...
            "calls_per_day": round(total_calls / (end_date - start_date).days, 2)
