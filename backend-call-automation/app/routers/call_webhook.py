...
@router.post("/webhook", tags=["Webhooks"]) # Asegúrate que la ruta coincida con la configurada en ElevenLabsService
async def handle_call_webhook(
    ...
    try:
        ...
        logger.info(f"Webhook para call_id: {call_id}, Status: {call_status}, Input: '{user_input}'")
        ...
        # 5. Manejar Otros Eventos
            ...
            logger.info(f"Evento intermedio para llamada {call_id}, estado: {call_status}. No se requiere acción.")
        ...
        logger.exception(f"Error fatal procesando webhook para call_id {call_id}: {str(e)}")
