...
from app.dependencies.service_dependencies import get_call_service
...
async def twilio_webhook(request: Request, call_service: CallService = Depends(get_call_service)):
    """
    ...
    await call_service.update_call(call.id, {"status": CallStatus.IN_PROGRESS})
    ...
    # Saludar al contacto
    response.say(
        f"Hola {contact.name}, le llamamos de {settings.APP_NAME}.", voice="woman", language="es-ES"
    )

    # Reproducir el script de la llamada
    response.say(call.script_template, voice="woman", language="es-ES")
...
async def twilio_gather_webhook(
    ...
    if digits == "1" or (speech_result and "interesado" in speech_result.lower()):
        ...
        await call_service.update_call_notes(call.id, "Usuario interesado. Requiere seguimiento.")
        ...
        # Actualizar la llamada con la respuesta del usuario
        await call_service.update_call_notes(call.id, "Usuario solicita ser contactado más tarde.")
        ...
        # Actualizar la llamada con la respuesta del usuario
        await call_service.update_call_notes(call.id, f"Respuesta no reconocida: {user_input}")
