...
@router.post("/sync")
async def force_sync(table_name: str, key_prefix: str | None = None) -> dict:
...
async def preload_conversation(conversation_id: str) -> dict:
    """Precarga una conversación desde Supabase a la caché."""
    success = await cache_service.preload_conversation(conversation_id)
    if not success:
        raise HTTPException(
            status_code=404, detail="Conversación no encontrada o error al precargar"
        )
