...
async def login(
    credentials: UserLogin, supabase_client: SupabaseClient = Depends(get_supabase_client)
) -> TokenResponse:
    """
...
async def register(
    user_data: UserRegister, supabase_client: SupabaseClient = Depends(get_supabase_client)
) -> UserResponse:
    """
    ...
    try:
        ...
        user = await auth_service.create_user(
            ...
            user_metadata={"name": user_data.name, "phone": user_data.phone},
        )
...
async def refresh_token(
    ...
    try:
        ...
        logger.error(f"Error al refrescar token: {e!s}")
        raise HTTPException(
...
@router.post("/logout")
async def logout(supabase_client: SupabaseClient = Depends(get_supabase_client)) -> dict[str, bool]:
    """
    ...
    try:
        ...
        logger.error(f"Error al cerrar sesión: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_current_user(
    ...
    try:
        ...
        logger.error(f"Error al obtener usuario actual: {e!s}")
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="No autenticado")
