...
from app.models.call import Call, CallCreate, CallDetail, CallStatus
...
async def create_call(
    ...
    call_service = CallService(
        supabase_client, twilio_service
    )  # Pasar twilio_service a CallService si es necesario (verificar constructor)
...
async def list_calls(
    campaign_id: str | None = Query(None, description="Filtrar por ID de campaña"),
    status: CallStatus | None = Query(None, description="Filtrar por estado de llamada"),
    contact_id: str | None = Query(None, description="Filtrar por ID de contacto"),
    start_date: datetime | None = Query(None, description="Filtrar desde fecha (formato ISO)"),
    end_date: datetime | None = Query(None, description="Filtrar hasta fecha (formato ISO)"),
...
async def update_call_status(
    ...
    duration: int | None = Query(None, description="Duración en segundos"),
    recording_url: str | None = Query(None, description="URL de la grabación"),
    error_message: str | None = Query(None, description="Mensaje de error"),
    call_service: CallService = Depends(get_call_service),
) -> Call:
    """
...
async def reschedule_call(
    ...
    return await call_service.reschedule_call(call_id=call_id, scheduled_time=scheduled_time)
...
async def get_call_metrics(
    ...
    return await call_service.get_call_metrics(
        campaign_id=campaign_id, start_date=start_date, end_date=end_date, group_by=group_by
    )


@router.post("/twilio_callback", status_code=status.HTTP_200_OK)
async def twilio_callback(
    twilio_data: dict[str, str], call_service: CallService = Depends(get_call_service)
) -> JSONResponse:
    """
    ...
    try:
        ...
        if not call:
            ...
            return JSONResponse(
                ...
                content={
                    "status": "warning",
                    "message": f"Llamada con SID {call_sid} no encontrada",
                },
            )

        # Extraer datos adicionales
        duration = (
            int(twilio_data.get("CallDuration", 0)) if twilio_data.get("CallDuration") else None
        )
        ...
        await call_service.update_call_status(
            ...
            error_message=error_message
            if error_message
            else (f"Error code: {error_code}" if error_code else None),
        )
        ...
        return JSONResponse(
            ...
            content={
                "status": "success",
                "message": f"Llamada {call_sid} actualizada a estado {mapped_status}",
            },
        )
...
async def handle_call_response(
    ...
    try:
        ...
        audio_response = await call_service.handle_call_response(
            call_id=call_id, user_message=message["text"]
        )
        ...
        raise HTTPException(
            ...
            detail=f"Error al procesar la respuesta: {e!s}",
        )
