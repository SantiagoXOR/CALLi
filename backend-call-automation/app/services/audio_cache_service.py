...
class AudioCacheService:
    ...
    def _save_metadata(self, metadata: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"Error al guardar metadatos de caché de audio: {e!s}")
    ...
    def _load_metadata(self) -> dict[str, Any]:
        ...
        """
        try:
            with open(os.path.join(self.cache_dir, "metadata.json")) as f:
                return json.load(f)
        except Exception as e:
            logger.error(f"Error al cargar metadatos de caché de audio: {e!s}")
    ...
    async def get_from_cache(self, text: str, voice_id: str, language: str = "es") -> str | None:
        """
        ...
        try:
            ...
            logger.error(f"Error al buscar audio en caché: {e!s}")
            return None

    async def save_to_cache(
        self, text: str, voice_id: str, audio_data: bytes, language: str = "es"
    ) -> str:
        """
        ...
        try:
            ...
            logger.error(f"Error al guardar audio en caché: {e!s}")
            return ""

    async def save_file_to_cache(
        self, text: str, voice_id: str, file: UploadFile, language: str = "es"
    ) -> str:
        """
        ...
        try:
            ...
            logger.error(f"Error al guardar archivo en caché: {e!s}")
            return ""
    ...
    async def remove_from_cache(self, cache_key: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error al eliminar audio de caché: {e!s}")
            return False

    async def _cleanup_if_needed(self, new_file_size: int) -> None:
        """
        ...
        try:
            ...
            logger.error(f"Error al limpiar caché de audio: {e!s}")
    ...
    async def get_cache_stats(self) -> dict[str, Any]:
        ...
        """
        if not self.enabled:
            return {"enabled": False, "message": "El caché de audio está desactivado"}
        ...
        try:
            ...
            usage_percent = (
                (total_size / self.max_cache_size) * 100 if self.max_cache_size > 0 else 0
            )
            ...
            top_accessed = sorted(
                metadata["files"].items(), key=lambda x: x[1]["access_count"], reverse=True
            )[:5]
            ...
            logger.error(f"Error al obtener estadísticas de caché de audio: {e!s}")
