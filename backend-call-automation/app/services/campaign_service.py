...
class CampaignService:
    ...
    async def create_campaign(self, campaign: CampaignCreate) -> Campaign:
        ...
        try:
            result = (
                self.supabase.from_table(self.table_name).insert(campaign.model_dump()).execute()
            )
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error al crear la campaña: {e!s}",
            )
    ...
    async def get_campaign(self, campaign_id: UUID) -> Campaign:
        ...
        try:
            result = (
                self.supabase.from_table(self.table_name)
                .select("*")
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error al obtener la campaña: {e!s}",
            )
    ...
    async def list_campaigns(
        ...
        try:
            ...
            # Usar directamente el código de estado en lugar de acceder a través de CampaignStatus
            raise HTTPException(
                status_code=500,  # HTTP_500_INTERNAL_SERVER_ERROR
                detail=f"Error al listar las campañas: {e!s}",
            )
    ...
    async def update_campaign(self, campaign_id: UUID, campaign_update: CampaignUpdate) -> Campaign:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error al actualizar la campaña: {e!s}",
            )
    ...
    async def delete_campaign(self, campaign_id: UUID) -> bool:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error al eliminar la campaña: {e!s}",
            )
    ...
    async def update_campaign_stats(
        ...
        try:
            ...
            if successful_calls + failed_calls > total_calls or any(
                x < 0 for x in [total_calls, successful_calls, failed_calls, pending_calls]
            ):
                raise HTTPException(
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error al actualizar las estadísticas: {e!s}",
            )
