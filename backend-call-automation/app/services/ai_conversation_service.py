...
class AIConversationService:
    ...
    async def process_message(
        ...
        async with self._rate_limit_semaphore:
            try:
                ...
                cached_history = (
                    await self.get_from_cache(generate_conversation_cache_key(conversation_id))
                    if conversation_id
                    else None
                )
                ...
                if conversation_id:
                    await self.save_conversation_metrics(
                        conversation_id, message, response, input_sentiment, response_sentiment
                    )
                ...
                logger.error(f"Error procesando mensaje: {e!s}")
    ...
    async def analyze_sentiment(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            response_str = (
                str(response) if not isinstance(response, (str, bytes, bytearray)) else response
            )
    ...
    async def save_conversation_metrics(
        ...
        response_sentiment: dict[str, Any] | None = None,
    ) -> None:
        ...
        interaction_data = {
            ...
            "response_sentiment": response_sentiment["primary_emotion"]
    ...
    def extract_conversation_context(self, memory: ConversationBufferMemory) -> dict[str, Any]:
