...
def manage_snapshot_file(tmp_path):
    ...
    with patch('scripts.rollback_utils.SNAPSHOT_DIR', test_snapshot_dir), \
         patch('scripts.rollback_utils.SNAPSHOT_FILE', test_snapshot_file):
...
def mock_redis_sync():
    ...
    with patch('scripts.rollback_utils.redis_client', mock) as patched_mock:
        ...
        monkeypatch.setattr(asyncio, 'iscoroutinefunction', lambda func: False if func == mock.delete else asyncio.iscoroutinefunction(func))
        yield patched_mock
        monkeypatch.undo()
...
def mock_connection_pool():
    ...
    mock_pool_instance = AsyncMock(spec_set=['close_all', 'get_active_connections', 'terminate_connection'])
...
class TestRollbackUtils:
    async def test_take_config_snapshot_success(self, mock_settings_object, manage_snapshot_file):
        ...
        with open(manage_snapshot_file, "r") as f:
    ...
    async def test_clear_redis_cache_async_found(self, mock_redis_async):
        ...
        mock_redis_async.delete.assert_has_calls([call("elevenlabs:a1"), call("elevenlabs:a2")], any_order=True)
    ...
    async def test_reset_prometheus_metrics_unregisters(self, mock_prometheus_registry):
        collector1 = MagicMock(spec=Collector, _name="elevenlabs_requests_total")
        collector2 = MagicMock(spec=Collector, _name="elevenlabs_pool_usage_ratio")
        mock_prometheus_registry._names_to_collectors = {
            "elevenlabs_requests_total": collector1,
            "elevenlabs_pool_usage_ratio": collector2,
            "other_metric": MagicMock(spec=Collector, _name="other_metric")
        }
    ...
    async def test_reset_prometheus_metrics_specific_and_missing(self, mock_prometheus_registry):
        collector1 = MagicMock(spec=Collector, _name="metric_exists")
    ...
    async def test_close_all_connections_uses_fallback(self, mock_connection_pool):
        ...
        mock_connection_pool.terminate_connection.assert_has_calls([call(mock_conn1), call(mock_conn2)], any_order=True)
    ...
    async def test_restore_config_to_defaults_no_snapshot(self, mock_settings_object, manage_snapshot_file):
    ...
    async def test_restore_config_to_defaults_snapshot_error(self, mock_settings_object, manage_snapshot_file):
        with open(manage_snapshot_file, "w") as f:
