...
async def test_initiate_outbound_call_failure(call_service):
    ...
    call_service.elevenlabs_service.initiate_outbound_call.side_effect = Exception(
        "Error de prueba"
    )
...
async def test_get_voice_for_call(call_service):
    ...
    call_service.supabase.table.return_value.select.return_value.eq.return_value.execute.return_value = MagicMock(
...
async def test_get_voice_for_call_fallback_to_default(call_service, monkeypatch):
    ...
    # Mockear settings
    monkeypatch.setattr(
        "app.config.settings.settings.ELEVENLABS_DEFAULT_VOICE_ID", default_voice_id
    )
...
async def test_handle_audio_stream_success(call_service):
    ...
    call_service.get_voice_for_call.assert_called_once_with(call_id)
    call_service.elevenlabs_service.generate_stream.assert_called_once_with(
...
async def test_update_call_history(call_service):
    ...
    call_service.update_call = (
        AsyncMock()
    )  # Mockear update_call para evitar llamadas reales a Supabase
...
async def test_create_call(call_service):
    ...
    # Verificar
    call_service.elevenlabs_service.generate_audio.assert_called_once_with(
        call_data.script_template
    )
...
async def test_get_call_success(call_service):
    ...
    call_service.supabase.table.return_value.select.return_value.eq.assert_called_once_with(
        "id", str(call_id)
    )
...
async def test_update_call_success(call_service):
    ...
    call_service.supabase.table.return_value.update.return_value.eq.assert_called_once_with(
        "id", str(call_id)
    )
...
async def test_delete_call_success(call_service):
    ...
    mock_delete_execute = AsyncMock(
        return_value=MagicMock(data=[mock_existing_call])
    )  # Supabase delete devuelve los datos eliminados
    call_service.supabase.table.return_value.delete.return_value.eq.return_value.execute = (
        mock_delete_execute
    )
    ...
    call_service.supabase.table.return_value.delete.return_value.eq.assert_called_once_with(
        "id", str(call_id)
    )
...
async def test_handle_call_response_audio_version_success(call_service):
    ...
    # Mockear dependencias
    mock_call = MagicMock(
        spec=Call, id=call_id, campaign_id="c1", contact_id="p1", interaction_history=[]
    )
    ...
    call_service.update_call_history.assert_called_once_with(
        call_id, user_message, ai_response_text
    )
