...
def parse_metrics_text(metrics_text: str) -> dict[str, Any]:
    ...
    for line in lines:
        ...
        # Si es parte de una métrica existente (tiene espacios al inicio)
            ...
            if not isinstance(metrics[current_metric], list):
                metrics[current_metric] = [str(metrics[current_metric])]

            # Ahora podemos estar seguros de que es una lista
            metric_list = metrics[current_metric]
            if isinstance(metric_list, list):  # Esta comprobación es para mypy
                metric_list.append(line.strip())
...
def assert_metric_value(
    metrics: dict[str, Any], metric_name: str, expected_value: int | float | str
) -> bool:
    """
    ...
    Args:
        metrics: Diccionario de métricas
        metric_name: Nombre de la métrica a verificar
        expected_value: Valor esperado (numérico o string)
    ...
    # Asegurarse de que la comparación devuelva un booleano
    result = metrics[metric_name] == expected_value
    return bool(result)
