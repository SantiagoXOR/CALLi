...
from tests.utils.metrics_helpers import parse_metrics_text, assert_metric_present, assert_metric_value
...
async def mocked_elevenlabs_service():
    ...
    with patch('app.services.elevenlabs_service.secrets_manager.get_elevenlabs_credentials', new_callable=AsyncMock, return_value={"api_key": "fake_integration_key"}), \
        ...
        mock_convo_instance.stream_audio = AsyncMock(return_value=b"mock_stream_response") # Mock stream if needed
...
"""
    ...
    @pytest.mark.asyncio
    async def test_audio_generation_flow(self, mocked_elevenlabs_service: ElevenLabsService, metrics_text: str):
        ...
        try:
            ...
            assert assert_metric_present(metrics, "elevenlabs_requests_total", 
                                       {"method": "generate_audio", "status": "success"})
            assert assert_metric_present(metrics, "elevenlabs_generation_duration_seconds",
    ...
    @pytest.mark.asyncio
    async def test_metrics_on_error(self, mocked_elevenlabs_service: ElevenLabsService, metrics_text: str):
        ...
        with patch("app.services.elevenlabs_service.generate", side_effect=Exception("Test error")):
            ...
            assert assert_metric_present(metrics, "elevenlabs_errors_total",
    ...
    async def test_concurrent_requests(self, mocked_elevenlabs_service: ElevenLabsService):
        ...
        with patch("app.services.elevenlabs_service.generate") as mock_generate:
            ...
            try:
                ...
                assert mock_generate.call_count == num_requests # Verify generate was called 5 times
    ...
    async def test_retry_mechanism_integration(self, mocked_elevenlabs_service: ElevenLabsService):
        ...
        with patch("app.services.elevenlabs_service.generate") as mock_generate:
            ...
            with patch('app.services.elevenlabs_service.secrets_manager.get_elevenlabs_credentials', new_callable=AsyncMock, return_value={"api_key": "mock_api_key"}), \
                 patch('app.services.elevenlabs_service.set_api_key'):
    ...
    @pytest.mark.asyncio
    async def test_generate_response_integration(self, mocked_elevenlabs_service: ElevenLabsService):
