...
def parse_metrics_text(metrics_text: str) -> dict[str, dict]:
    ...
    for line in metrics_text.split("\n"):
        if line.startswith("# TYPE"):
            ...
            metrics[current_metric] = {"type": parts[3], "help": "", "samples": []}
            ...
            if current_metric and current_metric in metrics:
                if "{" in line:  # Metric with labels
                    # Ignoramos el nombre ya que ya tenemos current_metric
                    _, rest = line.split("{", 1)
                    ...
                    metrics[current_metric]["samples"].append({"value": value, "labels": labels})
                else:  # Metric without labels
                    parts = line.split()
                    value = float(parts[1])
                    # Añadir a la lista de samples
                    metrics[current_metric]["samples"].append({"value": value, "labels": {}})

    return metrics


def assert_metric_present(
    metrics_data: dict[str, dict], metric_name: str, labels: dict[str, str] | None = None
) -> bool:
    """
