...
async def ai_service(mock_llm, mock_redis):
    service = AIConversationService(model_name="gpt-4", redis_client=mock_redis)
...
class TestAIConversationService:
    ...
    async def test_process_message_basic(self, ai_service):
        ...
        response = await ai_service.process_message(
            message="Hola, necesito información", conversation_id="test-123"
        )
    ...
    async def test_conversation_caching(self, ai_service, mock_redis):
        ...
        await ai_service.process_message(message=test_message, conversation_id=conv_id)
    ...
    async def test_context_handling(self, ai_service):
        ...
        context = {"campaign_id": "camp-123", "user_preferences": {"language": "es"}}

        response = await ai_service.process_message(
            message="Hola", conversation_id="test-123", context=context
        )
