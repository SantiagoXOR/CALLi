...
from isolated_ai_redis_tests import AIConversationService, generate_conversation_cache_key
    ...
    async def test_concurrent_load(self):
        ...
        async def conversation_simulation(conv_id):
            for i in range(messages_per_conv):
                response = await service.process_message(
                    f"Message {i}",
                    conversation_id=f"load-test-{conv_id}"
                )
                assert response is not None

        tasks = [
            conversation_simulation(i)
